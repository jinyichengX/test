...
// #define CELL_EMBED_NUM (EDGE_EMBED_NUM * (EDGE_MAX_SLOPE + 1)) /* 每光栅化一行最多只支持EDGE_EMBED_NUM个cell */
...
//         edge->x_full_step.frac = (edge->dx) % (edge->dy); //full frac is dy of edge，frac/dy是一个像素的小数部分，而不是子像素
//         /* 由y_start和y_end重置edge的y_start和y_end，先由扫描线的y跨度得出包围盒（连续方块模型）再裁剪edge */
//         if (edge->y_start < y_start) {
//             ipgui_scoord_t dy = y_start - edge->y_start; /* dy >= 0 */
//             ipgui_egde_xstep_t x_off;
//             s32_t temp;
//             /* 再修正起始x坐标，edge->x_cur at edge->y_start */
//             edge->y_start = y_start;

//             /* 修正整数部分 */
//             temp = dy / IPGUI_PIXEL_PRECI;/* xstep几个完整的像素 */
...
//             temp = (dy * edge->dx) / edge->dy ;/* 不足一像素偏移的x子像素，0~IPGUI_PIXEL_PRECI */
//             temp = (temp * edge->dy) / IPGUI_PIXEL_PRECI;/* 放大至与frac相同，有精度损失 */
//             x_off.frac += temp;
...
//         edge->y_min = IPGUI_SCANY_MIN(edge->y_start);
//         edge->y_max = IPGUI_SCANY_MAX(edge->y_end);
//         edge->x_cur.inte = scan_from.x;/* 这里应该是有bug */
//         edge->x_cur.frac = 0;
...
//     x_iter = IPGUI_ABS(ix1 - ix2) + 1; /* x_iter个像素被影响了 */
//     while (x_iter --)
//     {
//         if (step == 1) { /* dx > 0 */
//             if (ix1 != ix2) {
...
//         long long temp = dy * edge->dx;
//         x_step.inte = temp / edge->dy;
...
//     s32_t ret;
//     ipgui_edge_dir_t dir;
//     ipgui_coord_t y; /* 扫描线y坐标 */

//     if (ras->err) {
//         // ipgui_dbg_error("rasterizer in error state\r\n"); /* 不能一直打印， 会影响性能 */
//         return IPGUI_ERR_NOMEM;
//     }
...
//     buffer.start.y <<= IPGUI_PIXEL_BITS; /* 扩大子像素精度倍 */
//     buffer.end.x <<= IPGUI_PIXEL_BITS;  /* 扩大子像素精度倍 */
//     buffer.end.y <<= IPGUI_PIXEL_BITS;  /* 扩大子像素精度倍 */
...
//     ras->y_min = IPGUI_SCANY_MIN(draw.start.y);/* 光栅化器的y跨度和包围盒的y跨度一样 */
//     ras->y_max = IPGUI_SCANY_MAX(draw.end.y);

//     /* 扫描线的y跨度（连续方块模型）扩大IPGUI_PIXEL_PRECI(64)倍 */
...
//         ipgui_polygon_ras_add_edge(ras, y_start, y_end, edge_p1, edge_p2, dir, (ipgui_edge_t **)0);
//     }

//     /* 还原draw */
//     draw.start.x >>= IPGUI_PIXEL_BITS; /* 缩小子像素精度倍 */
//     draw.start.y >>= IPGUI_PIXEL_BITS; /* 缩小子像素精度倍 */
//     draw.end.x >>= IPGUI_PIXEL_BITS;  /* 缩小子像素精度倍 */
//     draw.end.y >>= IPGUI_PIXEL_BITS;  /* 缩小子像素精度倍 */
...
//         {
//             next = iter->next;
//             /* for every edge, caculate covered pixel's left sign area */
...
//     ras->y_min = draw.start.y; /* 光栅化器的y跨度和包围盒的y跨度一样 */
...
//     last = avl_find_last_node(&ras->scanline_cells);
