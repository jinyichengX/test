...
{
    if (!surf || !icon || !style) return;

    ipgui_point_t pivot  = {0, 0};
    ipgui_point_t anchor = {x, y};

    ipgui_draw_icon(surf, (ipgui_aabb_t *)0, icon,
                    &pivot, &anchor,
                    (ipgui_trans_mat_t *)0,
    ...
    const ipgui_draw_icon_style_t   * style)
{
    ...
    ipgui_point_t pivot  = {icon->w / 2, icon->h / 2};
...
__IPGUI_STATIC__ void ipgui_icon_compute_align_offset(
...
{
    switch (align) {
        case IPGUI_ICON_ALIGN_TOP_LEFT:     * ox = 0;                           * oy = 0;                           return;
        case IPGUI_ICON_ALIGN_TOP:          * ox = (container_w - item_w) / 2;  * oy = 0;                           return;
        case IPGUI_ICON_ALIGN_TOP_RIGHT:    * ox = container_w - item_w;        * oy = 0;                           return;
        case IPGUI_ICON_ALIGN_LEFT:         * ox = 0;                           * oy = (container_h - item_h) / 2;  return;
        case IPGUI_ICON_ALIGN_CENTER:       * ox = (container_w - item_w) / 2;  * oy = (container_h - item_h) / 2;  return;
        case IPGUI_ICON_ALIGN_RIGHT:        * ox = container_w - item_w;        * oy = (container_h - item_h) / 2;  return;
        case IPGUI_ICON_ALIGN_BOTTOM_LEFT:  * ox = 0;                           * oy = container_h - item_h;        return;
        case IPGUI_ICON_ALIGN_BOTTOM:       * ox = (container_w - item_w) / 2;  * oy = container_h - item_h;        return;
        case IPGUI_ICON_ALIGN_BOTTOM_RIGHT: * ox = container_w - item_w;        * oy = container_h - item_h;        return;
    }
}
    ...
    const ipgui_draw_icon_style_t   * style)
{
    ...
    if (!surf || !icon || !target || !style) return;
    ...
    if (fit == IPGUI_ICON_FIT_NONE) {
        ...
        draw_w = iw;
        draw_h = ih;
        ipgui_icon_compute_align_offset(align, tw, th, draw_w, draw_h, &ox, &oy);
        ...
        ipgui_draw_icon(surf, (ipgui_aabb_t *)target, icon,
                        &pivot, &anchor,
                        (ipgui_trans_mat_t *)0,
                        (ipgui_draw_icon_style_t *)style);
        return;
    ...
    /* 需要缩放变换 */
    {
        ...
        ipgui_icon_compute_align_offset(align, tw, th, scaled_w, scaled_h, &ox, &oy);
