    ...
    ipgui_image_draw_style_t * style)
{
    ...
    if (trans->a * trans->d == trans->b * trans->c) {
        ipgui_dbg_warning("warning: the transfrom matrix err, because it is不可逆的\r\n");
        return;
    }

    ipgui_aabb_t img_aabb;
    ipgui_coord_t img_w, img_h;
    img_aabb = ipgui_calc_image_transformed_aabb_rel_to_pivot(pivot, trans, img_data->w, img_data->h);
