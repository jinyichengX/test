...
//单buffer，省RAM
static uint32_t fb_mem[1][FB_LCD_XSIZE * FB_LCD_YSIZE / 4 * LCD_BYTE_PP] __attribute__((aligned(32)));
...
inline static void f1c100s_debe_set_mode(fb_f1c100s_priv_data_t* pdat)
{
  ...
  //write32((uint32_t)&debe->layer0_attr1_ctrl, (0x05 << 8) | (1 << 2));  /* 此处设置像素格式 bgr565 */

  write32((uint32_t)&debe->layer0_attr1_ctrl, (0x05 << 8) | (0 << 2));  /* chengju 此处设置像素格式 rgb565 */
...
{
  // enable SCLK_GATING
  gate_clk_enable(0xff820118, 31, false);
  // enable LCD_GATING
  gate_clk_enable(0xff820064, 4, false);
}

inline static void f1c100s_clk_tcon_disable()
{
  gate_clk_disable(0xff820118, 31, false);
  gate_clk_disable(0xff820064, 4, false);
}
...
static void f1c100s_clk_defe_init(void)
{
  ...
  clk_mux_set_parent(0xff82010c, 3, 24, 0);
  // set defe Clock divide ratio
  clk_divider_set_rate(0xff82010c, 4, 0, true, f1c100s_tcon_clk_get_rate(), f1c100s_tcon_clk_get_rate());
}

inline static void f1c100s_clk_defe_enable()
{
  // enable SCLK_GATING
  gate_clk_enable(0xff82010c, 31, false);
  // enable DEFE_GATING
  gate_clk_enable(0xff820064, 14, false);
}

inline static void f1c100s_clk_defe_disable()
{
  gate_clk_disable(0xff82010c, 31, false);
  gate_clk_disable(0xff820064, 14, false);
}
// "clk-mux@0x01c2010c": {
//  "parent": [
...
static void f1c100s_clk_debe_init(void)
{
  ...
  clk_mux_set_parent(0xff820104, 3, 24, 0);
  // set debe Clock divide ratio
  clk_divider_set_rate(0xff820104, 4, 0, true, f1c100s_tcon_clk_get_rate(), f1c100s_tcon_clk_get_rate());
}
...
{
  // enable SCLK_GATING
  gate_clk_enable(0xff820104, 31, false);
  // enable DEBE_GATING
  gate_clk_enable(0xff820064, 12, false);
}

inline static void f1c100s_clk_debe_disable()
{
  gate_clk_disable(0xff820104, 31, false);
  gate_clk_disable(0xff820064, 12, false);
}
...
//#endif
 ...
 // spl_prints("fb init ##\r\n", 0);
  ...
  pwm_f1c100s_init(pdat->backlight);//chengju实际板子PE6与LCD调光电路是断开的，该脚被CSI口的D3数据脚使用
}
...
void fb_f1c100s_present(void)
{
	framebuffer_t* fb = &fbuf;
	int index = 0;
  fb_f1c100s_priv_data_t* pdat = (fb_f1c100s_priv_data_t*)fb->priv;
...
void InitLcdInf(void * p)
{
	...
	pinf->buf = (uint32_t *)fb_mem[0];
	pinf->deep = 16;
	//pinf->format = FORMAT_ARGB8888;
	pinf->fmt = FORMAT_RGB565;                   //FORMAT_RGB565
	pinf->h = LCD_PANEL_YSIZE;
	pinf->w = LCD_PANEL_XSIZE;
...
void fb_f1c100s_init(Canvas_t * pinf)
{
	framebuffer_t* fb;
  int32_t i;
 ...
 // spl_prints("fb init ##\r\n", 0);
  ...
  if(pinf) {InitLcdInf(pinf);}     //chengju240925



  LCD_FillRect(0, 0, pinf->w, pinf->h, 0xffff);  //测试白屏
...
void Fill_Screen(uint16_t color, int index){ //屏幕填充函数



	uint16_t * p = (uint16_t *)fb_mem[index];



     for (int i = 0; i < (LCD_PANEL_XSIZE*LCD_PANEL_YSIZE);i++)
        {
            *p =color;
            p ++;
        }
...
void LCD_DrawPoint(int16_t x, int16_t y, uint16_t color)
{
	...
	volatile uint16_t * p = (uint16_t *)fb_mem[0];


	t = LCD_PANEL_XSIZE * y + x;

	if(t < LCD_PANEL_XSIZE*LCD_PANEL_YSIZE)
	{
		p += t;
		*p =color;
	}
...
void LCD_CopyRect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t * pc)
{
