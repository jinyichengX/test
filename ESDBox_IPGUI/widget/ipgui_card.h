#ifndef IPGUI_CARD_H
#define IPGUI_CARD_H

#include "ipgui_widget.h"
#include "ipgui_box_style.h"
#include "ipgui_draw_box_background.h"
#include "ipgui_draw_box_border.h"
#include "ipgui_draw_box_shadow.h"

#ifdef __cplusplus
extern "C" {
#endif

typedef struct {
    ipgui_box_style_t        shape;
    ipgui_box_bg_style_t     bg;
    ipgui_box_border_style_t  border;
    ipgui_box_shadow_style_t  shadow;
    int                      has_shadow;
    int                      image_height;  /* 顶部图片区域高度 */
    ipgui_box_bg_style_t     img_area_bg;   /* 图片占位背景 */
} ipgui_card_style_t;

typedef struct {
    ipgui_widget_t      base;
    ipgui_card_style_t  style;
} ipgui_card_t;

extern __IPGUI_API__ ipgui_card_t * ipgui_card_create(ipgui_widget_t * parent);
extern __IPGUI_API__ void ipgui_card_style_init(ipgui_card_style_t * s);

#ifdef __cplusplus
}
#endif
#endif
